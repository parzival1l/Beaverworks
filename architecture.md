...
Last updated: 2026-05-02 (Express→agent bridge: `adk workflows run` CLI; RAG search + CRA parser; secrets via `npm run secrets:sync`)
...
| `/dashboard` | Dashboard | `?mode=all` \| `filtered`; tax optimizer sidebar; `filtered` calls `/api/search` |
