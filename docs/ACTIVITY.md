...
- **Files:** `README.md`, `docs/ACTIVITY.md`
- **What:** Replaced README opening with user-centric framing (matching, Québec/Canada, tax calculator, transparency of public-record data); stripped demo sign-in and stack jargon from the hero paragraph and kept monorepo layout in a separate sentence for contributors.
- **Why:** Opening should read trust and usefulness to donors before implementation detail.
- **Commands:** none

## 2026-05-02 19:02 ET — Housekeeping: move SSOT architecture + activity log under `docs/`

- **Files:** `git mv architecture.md docs/architecture.md`, `git mv ACTIVITY.md docs/ACTIVITY.md`, `docs/{architecture,feature-process,local-testing}.md`, `AGENTS.md`, `README.md`, `.cursor/rules/{core,git}.mdc`, `backend/AGENTS.md`, `frontend/AGENTS.md`
- **What:** Consolidated architecture SSOT and the session activity log under `docs/` alongside the other process docs; rewired every cross-link and Cursor rule path; README/AGENTS entrypoints now point at `docs/architecture.md` and `docs/ACTIVITY.md`.
- **Why:** Match the goal of an uncluttered repo root (only `README.md` + `AGENTS.md` as top-level Markdown pointers).
- **Commands:** none
...
- **What:** Relocated phase tracker and Markdown Botpress ADK companion out of repo root; refreshed links and project tree in architecture SSOT (subsequently both architecture and this log also moved under `docs/` — see next entry).
- **Why:** Reduce root clutter without changing tooling contracts.
- **Commands:** none

## 2026-05-02 — README: app preview image

- **Files:** `README.md`, `docs/assets/altru-app-preview.png`, `docs/ACTIVITY.md`
...
- **Files:** `scripts/launch-dev.sh`, `docs/local-testing.md`, `scripts/test-rag-pathway.sh` (header comment), `docs/ACTIVITY.md`
...
- **Files:** `docs/local-testing.md`, `docs/ACTIVITY.md`
...
- **Files:** `frontend/src/components/charity/CharityCard.tsx`, `frontend/src/setupTests.ts`, `frontend/src/routerFuture.ts`, `frontend/src/main.tsx`, `frontend/src/__tests__/{DashboardPage,LoginPage}.test.tsx`, `frontend/package.json`, `package-lock.json`, `docs/architecture.md`
...
- **Files:** `docs/local-testing.md` (new), `scripts/{test-rag-pathway,launch-dev}.sh` (comment pointers), `docs/architecture.md` (Testing link + tree)
...
## 2026-05-02 — Phase 5 tracker: reconcile `docs/feature-process.md` + mention `test-rag-pathway`

- **Files:** `docs/feature-process.md`, `docs/architecture.md`, `docs/ACTIVITY.md`
- **What:** **docs/feature-process.md** was stale: it still said the Vite proxy targeted **3001** and marked **RAG as pending** after `/api/search` and the Botpress workflow had already shipped. Corrected Phase 2 (proxy → **3002**, RAG ✅, noted `scripts/test-rag-pathway.sh`). Added an explicit Phase 5 table so "docs + tracker" lines up with the plan. Listed `test-rag-pathway.sh` under **docs/architecture.md** Testing + Project Structure.
- **Why:** Cursor's Phase 5 todo looked unfinished because **the todo wasn't flipped**, not because **docs/architecture.md** was blank — SSOT already described the bridge.
- **Commands:** none
...
- **Files:** `agent/DEMO-SCRIPT-60s.md`, `docs/architecture.md`, `docs/ACTIVITY.md`
- **What:** Refreshed the 60s Botpress talk track (demo auth on client, frontend keyword fallback on search errors, Vite :5173 in demo beats, technical note on `adk workflows run`). Corrected `docs/architecture.md` API behaviour and Mermaid edge: Express uses CLI bot client, not `BOTPRESS_WORKFLOW_URL` HTTP.
- **Why:** Docs matched an older integration story; implementation is `backend/src/bot/client.ts` shelling out to ADK.

---

## 2026-05-02 — agent: `secrets:sync` before `adk dev`

- **Files:** `agent/scripts/sync-secrets.ts` (new), `agent/package.json`, `agent/README.md`, `docs/architecture.md`, `docs/ACTIVITY.md`
...
- **Files:** `backend/` (Express `POST /api/questionnaire/submit`, Jest tests), `frontend/src/api/questionnaire.ts`, `frontend/vite.config.ts`, `frontend/src/vite-env.d.ts`, `.gitignore`, `docs/architecture.md`, `docs/feature-process.md`, `docs/ACTIVITY.md`
...
- **Files:** `frontend/` (UI, auth key, demo email, package name), `docs/architecture.md`, `docs/ACTIVITY.md`
...
- **Files:** `frontend/` (new), `docs/architecture.md`, `docs/ACTIVITY.md`, `docs/feature-process.md`
...
- **Files:** `.cursor/rules/core.mdc`, `.cursor/rules/git.mdc`, `docs/architecture.md`, `docs/ACTIVITY.md`, `docs/feature-process.md`, `AGENTS.md`, `backend/AGENTS.md`, `frontend/AGENTS.md`
- **What:** Established best-practice scaffolding matching the HailBairApp and food-chatbot project patterns: TDD-enforcing Cursor rules (`core.mdc` + `git.mdc`), architecture SSOT (`docs/architecture.md`), session activity log (`docs/ACTIVITY.md`), phase-level feature tracker (`docs/feature-process.md`), and updated AGENTS.md files at root, backend, and frontend.
